...
const prepStops = (stops?: string[]) => {
  const base = (stops && stops.length ? stops : ['#4b3621', '#2b2118', '#8B5A2B', '#3b2d21']).slice(0, MAX_COLORS);
  if (base.length === 1) base.push(base[0]);
  while (base.length < MAX_COLORS) base.push(base[base.length - 1]);
  const arr: [number, number, number][] = [];
  for (let i = 0; i < MAX_COLORS; i++) arr.push(hexToRGB(base[i]));
  const count = Math.max(2, Math.min(MAX_COLORS, stops?.length ?? base.length));
  return { arr, count };
};
...
const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  useEffect(() => {
    ...
    if (mixBlendMode) canvas.style.mixBlendMode = mixBlendMode as any;
...
`;
    ...
    return () => {
      ...
      (geometryRef.current as any)?.remove?.();
      (meshRef.current as any)?.remove?.();
      (rendererRef.current as any)?.destroy?.();
  ...
  return (
    <div
      ref={containerRef}
      // Fixed fullscreen canvas; RN Web will happily render this DOM node
      style={{
        position: 'fixed',
        inset: 0,
        width: '100vw',
